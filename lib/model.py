...
def convert_ldm_vae_checkpoint(checkpoint, config):
    ...
    num_mid_res_blocks = len({'.'.join(key.split('.')[:3]) for key in vae_state_dict if "encoder.mid.block" in key})
    ...
    num_mid_res_blocks = len({'.'.join(key.split('.')[:3]) for key in vae_state_dict if "decoder.mid.block" in key})
    for i in range(1, num_mid_res_blocks + 1):
...
def load_target_model(pretrained_model_name_or_path, clip_dir):
    _, state_dict = load_checkpoint(pretrained_model_name_or_path)
    ...
    text_model = CLIPTextModel.from_pretrained(clip_dir)
